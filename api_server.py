...
def analyze_file():
    ...
    try:
        ...
        original_upload_result = cloudinary.uploader.upload(
            ...
            # Use a short, unique UUID as the public_id to prevent length errors
            public_id=f"original_{uuid.uuid4().hex}", 
            resource_type="auto", 
            overwrite=True
        )
...
def get_user_stats():
    ...
    try:
        ...
        print(f"ERROR in /api/stats/user: {e}"); traceback.print_exc()
...
def get_history():
    ...
    try:
        ...
        print(f"ERROR in /api/history: {e}"); traceback.print_exc()
