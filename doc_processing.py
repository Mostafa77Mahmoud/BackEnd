...
from config import CLOUDMERSIVE_API_KEY
from utils import ensure_dir, clean_filename, clean_model_response
...
def build_structured_text_for_analysis(doc: DocxDocument) -> tuple[str, str]:
    ...
    """
    # ... (rest of the function is the same, no print statements here)
    structured_markdown = []
    plain_text_parts = []
...
# ... (Helper functions like set_cell_direction_rtl, _parse_markdown_to_parts_for_runs, etc. are unchanged)
...
def create_docx_from_llm_markdown(
    ...
    try:
        ...
        if isinstance(terms_for_marking, list) and terms_for_marking: 
            logger.info(f"Using new list-based term marking logic for DOCX. {len(terms_for_marking)} terms.")
            # ... (The logic for processing and marking terms remains the same)
            ...
            for term_idx, term_data in enumerate(terms_for_marking):
                ...
                try:
                    ...
                    logger.warning(f"Regex error for term '{term_data.get('term_id')}': {re_err}. Pattern: {term_text_pattern}")
                    logger.info(f"Falling back to string.find for term: '{term_text_original[:50]}...'")
                ...
                if match:
                    ...
                    if inter_term_text.strip():
                        logger.debug(f"Rendering inter-term text (len: {len(inter_term_text)}): '{inter_term_text[:50].strip()}...'")
                    ...
                    logger.debug(f"Found term '{term_data.get('term_id')}' at pos {found_pos_absolute}: '{term_text_original[:50].strip()}...'")
                    ...
                    if is_confirmed and confirmed_text_content and \
                       not initial_is_valid and confirmed_text_content.strip() != term_text_original.strip():
                        logger.debug(f"Applying DOCX MARKING: Red original, Green new for term {term_data.get('term_id')}")
                        ...
                        if is_confirmed and confirmed_text_content:
                            ...
                            logger.debug(f"Applying DOCX MARKING: Green (confirmed) for term {term_data.get('term_id')}")
                        elif not initial_is_valid:
                            final_text_color_for_term = RGBColor(255,0,0) 
                            logger.debug(f"Applying DOCX MARKING: Red (initially invalid) for term {term_data.get('term_id')}")
                    ...
                    logger.warning(f"Term '{term_data.get('term_id')}' text not found sequentially from pos {search_start_pos}: '{term_text_original[:50].strip()}...'")
            
            if current_markdown_pos < len(processed_markdown_text):
                remaining_text = processed_markdown_text[current_markdown_pos:]
                logger.debug(f"Rendering remaining_text (len: {len(remaining_text)}): '{remaining_text[:50].strip()}...'")
            ...
            logger.info(f"Using old dict-based or no-term marking logic. terms_for_marking type: {type(terms_for_marking)}")
            # ... (rest of this logic is the same)
        ...
        # ... (Signature block logic is the same)
        ...
        logger.error(f"❌ ERROR creating DOCX from LLM markdown ({contract_language}): {e}", exc_info=True)
        raise ValueError(f"فشل إنشاء DOCX: {e}")


def convert_docx_to_pdf_cloudmersive(docx_path: str, output_folder: str) -> str:
    """
    Converts a DOCX file to PDF using the Cloudmersive API.
    ...
    try:
        with open(docx_path, 'rb') as f:
            files = {'inputFile': (os.path.basename(docx_path), f, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')}
            
            logger.info(f"Attempting PDF conversion with Cloudmersive API for: {docx_path}")
        ...
        if os.path.exists(pdf_output_path) and os.path.getsize(pdf_output_path) > 0:
            ...
            raise Exception("PDF file not created or is empty after Cloudmersive API call.")

    except requests.exceptions.RequestException as e:
        logger.critical(f"❌ Cloudmersive API request failed: {e}")
        if e.response is not None:
            logger.error(f"Cloudmersive API Response Status: {e.response.status_code}, Body: {e.response.text}")
        raise Exception(f"PDF conversion failed due to a network error or API issue: {e}")
    except Exception as e:
        logger.error(f"❌ An unexpected error occurred during PDF conversion for {docx_path}: {e}", exc_info=True)
